...
import com.asiapay.paydollar.apppay.Scanner.Fail_word;
...
//        errAdvise = intent.getStringExtra(Constants.ERRMSG)==null?"":Fail_word.failadvise( intent.getStringExtra(Constants.ERRMSG), Fail_scan.this );
        ...
        btnCheck.setOnClickListener(new Button.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                QRPay alipayTH = new QRPay(ScanQR_Failure.this, progressDialog_ALIPAYTH, getPrefPayGate(), true);
...
//                String deviceSerial = db.getSingleData(Constants.DB_TABLE_ID, Constants.DB_DEVICE_SERIALNO, Constants.DB_MERCHANT_ID, whereargs);
                ...
                alipayTH.execute(amount, merrequestamt, Surcharge, Currcode, merchant_ref_no, auth_code, pMethod, merchantId, merName, userID, String.valueOf(hideSurcharge), autoBBLQRTraceNo(), autoBBLQRBatchID(), deviceSerial);
            }

        });

        if(pMethod.equalsIgnoreCase("ALIPAYTHOFFL")) {
            btnMainMenu.setText("Cancel");
    ...
    public void scanback(String successcode,
        ...
        incMerRef();
        Toast.makeText(ScanQR_Failure.this, "Success Scan", Toast.LENGTH_SHORT).show();
        Intent intent = new Intent();
